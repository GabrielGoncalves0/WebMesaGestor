...
{
    public class TransacaoEdicaoValidator : AbstractValidator<TraEdicaoDTO>
    {
        public TransacaoEdicaoValidator()
        {
            RuleFor(x => x.Id)
                    .NotEmpty().WithMessage("O ID da transação é obrigatório.")
                    .NotEqual(Guid.Empty).WithMessage("ID da transação inválido.");
